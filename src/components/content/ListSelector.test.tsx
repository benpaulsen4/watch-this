...
describe("ListSelector", () => {
    ...
    const fetchMock = vi.fn(
      async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (addMatch && method === "POST") {
          ...
          return {
            ok: true,
            json: async () => ({ id: listsWithContent[listId] }),
          } as Response;
        }
  ...
  it("supports creating a new list and quick-adding content", async () => {
    const user = userEvent.setup();
    ...
    listsWithContent = { "list-tv": "item-xyz" } as any;
    ...
    await screen.findByText(/Manage Lists/i);
    const createBtn = screen.getByRole("button", { name: /Add to new list/i });
    await user.click(createBtn);
    await user.type(screen.getByLabelText(/List Name/i), "Quick Created");
