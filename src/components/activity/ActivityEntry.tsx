...
export function ActivityEntry({ activity, currentUsername }: ActivityEntryProps) {
  ...
  const getUsernameString = (activity: Activity, currentUsername: string) => {
    ...
    if (activity.isCollaborative && (activity.collaborators?.length ?? 0) > 0) {
      ...
      if (collaborators.length <= 3) {
        const lastCollaborator = replaceWithYou(collaborators[collaborators.length - 1].username);
