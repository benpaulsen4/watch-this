...
import { Check, ChevronDown, ChevronRight, Calendar, Clock, RotateCcw, Eye } from 'lucide-react';
...
export function EpisodeTracker({ tvShowId, className, onShowStatusChanged }: EpisodeTrackerProps) {
      ...
      const seasonsData: SeasonData[] = seasonResults
        ...
        .map(result => ({
          ...
          watchStatuses: watchStatuses.filter(
            (status: EpisodeWatchStatus) => status.seasonNumber === result.seasonNumber
          ),
          isExpanded: false
        }));
  ...
  const toggleSeasonWatched = async (seasonNumber: number, markAsWatched: boolean) => {
    ...
    try {
      const episodes = seasonData.season.episodes.filter(ep => new Date(ep.air_date) < new Date()).map(ep => ({
        seasonNumber,
  ...
  const getEpisodeWatchStatus = (seasonNumber: number, episodeNumber: number): boolean => {
    const seasonData = seasons.find(s => s.season.season_number === seasonNumber);
    if (!seasonData) return false;
    
    return seasonData.watchStatuses.some(
      status => status.seasonNumber === seasonNumber && 
  ...
  const getEpisodeWatchDate = (seasonNumber: number, episodeNumber: number): Date | null => {
    const seasonData = seasons.find(s => s.season.season_number === seasonNumber);
  ...
  const getSeasonProgress = (seasonData: SeasonData) => {
    ...
    const watchedEpisodes = seasonData.watchStatuses.filter(status => status.watched).length;
  ...
  return (
    <div className={cn('space-y-4', className)}>
      ...
      {seasons.map((seasonData) => {
        ...
        return (
          <div key={seasonData.season.season_number} className="bg-gray-800 rounded-lg border border-gray-700">
            {/* Season Header */}
            <div className="p-4">
              <div className="flex items-center justify-between">
                <button
                  onClick={() => toggleSeasonExpanded(seasonData.season.season_number)}
                ...
                <div className="flex flex-1 min-w-0 flex-wrap items-center justify-end gap-3">
                  ...
                  <div className="flex gap-1">
                    {!isFullyWatched && (
                      <Button
                        onClick={() => toggleSeasonWatched(seasonData.season.season_number, true)}
                        size="sm"
                        variant="outline"
                        className="text-xs"
                      >
                    ...
                    {progress.watched > 0 && (
                      <Button
                        onClick={() => toggleSeasonWatched(seasonData.season.season_number, false)}
                        size="sm"
                        variant="outline"
                        className="text-xs"
                      >
              ...
              <div className="border-t border-gray-700">
                <div className="p-4 space-y-2">
                  {seasonData.season.episodes.map((episode) => {
                    const isWatched = getEpisodeWatchStatus(seasonData.season.season_number, episode.episode_number);
                    const watchDate = getEpisodeWatchDate(seasonData.season.season_number, episode.episode_number);
