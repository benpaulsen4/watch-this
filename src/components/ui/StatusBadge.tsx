...
import { WatchStatusEnum, MovieWatchStatusEnum, TVWatchStatusEnum, ContentTypeEnum } from '@/lib/db/schema';
...
export function StatusBadge({ 
  ...
  if (contentType === 'movie') {
    const validMovieStatuses: MovieWatchStatusEnum[] = ['planning', 'completed'];
    if (!validMovieStatuses.includes(status as MovieWatchStatusEnum)) {
      console.warn(`Invalid movie status: ${status}`);
    }
  } else if (contentType === 'tv') {
    const validTVStatuses: TVWatchStatusEnum[] = ['planning', 'watching', 'paused', 'completed', 'dropped'];
 ...
 */
export function getAvailableStatuses(contentType: ContentTypeEnum): WatchStatusEnum[] {
  if (contentType === 'movie') {
